...
import { 
  Shield, Cpu, Users, Search, BrainCircuit, Play, Database, Sparkles, 
  Server, Cloud, Pickaxe, Activity, Lock, FileText, Globe, Layers, 
  Zap, ShieldCheck, Network
} from 'lucide-react';

const Strike48Presentation = () => {
  const [activeAgent, setActiveAgent] = useState('Cyra');
  const [scrolled, setScrolled] = useState(false);
  const [isMined, setIsMined] = useState(false);
  const [isMining, setIsMining] = useState(false);

  // Ref for Section 2 (Unified Layer)
  ...
  type Agent = { role: string; tagline: string; desc: string; task: React.ReactNode; color: string; image: string; icon: React.ReactNode };
    ...
    <div className="bg-slate-950 text-white font-sans overflow-x-hidden selection:bg-amber-500 selection:text-black">
      ...
      <header className="relative min-h-screen flex items-center justify-center pt-20 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <img src="https://images.unsplash.com/photo-1526374965328-7f61d4dc18c5?q=80&w=2670&auto=format&fit=crop" alt="Background" className="w-full h-full object-cover opacity-20" />
        ...
        <div className="max-w-5xl mx-auto px-6 text-center z-10 animate-fade-in flex flex-col items-center">
          ...
          <p className="text-xl md:text-2xl text-slate-300 mb-12 max-w-3xl mx-auto leading-relaxed text-shadow-sm">
          ...
          <div className="relative mt-8 group cursor-pointer" onClick={handleStrikeGold}>
            <div className={`absolute -top-16 -right-12 transition-transform duration-500 origin-bottom-left z-20 ${isMining ? 'rotate-[-45deg] translate-y-10 translate-x-[-10px]' : 'rotate-12 group-hover:rotate-0'}`}>
      ...
      <section ref={architectureRef} className="py-24 bg-slate-900 border-t border-slate-800 relative overflow-hidden">
        <div className="max-w-7xl mx-auto px-6 text-center">
          ...
          <p className="text-slate-400 mb-16 max-w-2xl mx-auto text-lg">
            Stop switching between tabs. Strike48 <span className="text-white font-bold">sits on top</span> of your fragmented silos, correlating data without moving it.
          </p>
          ...
          <div className="relative max-w-5xl mx-auto h-[450px] flex flex-col justify-end items-center">
            ...
            <div className="absolute top-0 z-20 w-full animate-fade-in">
              <div className="inline-block p-6 px-16 bg-slate-950 border-2 border-amber-500 rounded-3xl shadow-[0_0_50px_rgba(245,158,11,0.2)] relative">
                 ...
                 <div className="text-sm font-mono text-amber-500 mt-2 tracking-widest uppercase">Unified Intelligence Layer</div>
      ...
      {/* --- 4. SOLVING AI HALLUCINATION (Moved Up per flow) --- */}
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 flex flex-col lg:flex-row gap-16 items-center">
            ...
            <div className="flex flex-col gap-4">
              {Object.keys(agents).map((name) => (
                <button 
                  ...
                  className={`flex items-center gap-4 p-4 rounded-xl text-left transition-all border ${activeAgent === name ? 'bg-slate-800 border-amber-500/50 shadow-lg scale-105' : 'bg-slate-950/50 border-slate-800 hover:bg-slate-800'}`}
                >
                  <div className={`w-12 h-12 rounded-full overflow-hidden border-2 ${activeAgent === name ? 'border-amber-500' : 'border-slate-700'}`}>
                      <img src={agents[name].image} alt={name} className="w-full h-full object-cover filter brightness-90 contrast-110" />
                  </div>
                  <div>
                    <h4 className={`font-bold text-lg ${activeAgent === name ? 'text-white' : 'text-slate-400'}`}>{name}</h4>
                    <span className="text-xs font-mono text-slate-500 uppercase">{agents[name].role}</span>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="lg:w-7/12 w-full">
            <div className="relative bg-slate-950/80 backdrop-blur-xl border border-slate-700 rounded-3xl p-8 md:p-12 shadow-2xl h-[600px] flex flex-col justify-between overflow-hidden">
              <div className={`absolute -top-20 -right-20 w-64 h-64 ${agents[activeAgent].color.replace('text-', 'bg-')}/10 rounded-full blur-[80px]`}></div>
              <div>
                <div className="flex justify-between items-start mb-8">
                  <div className="flex items-center gap-6">
                    <div className="w-24 h-24 rounded-2xl overflow-hidden border-2 border-slate-700 shadow-xl">
                        <img src={agents[activeAgent].image} alt={activeAgent} className="w-full h-full object-cover filter brightness-90 contrast-110" />
                    </div>
                    <div>
                        <h3 className={`text-5xl font-bold ${agents[activeAgent].color}`}>{activeAgent}</h3>
                        <p className="text-lg font-mono text-slate-400 mt-2 flex items-center gap-2">
                           {agents[activeAgent].icon} {agents[activeAgent].tagline}
                        </p>
                    </div>
                  </div>
                </div>
        ...
        <div className="max-w-5xl mx-auto px-6 text-center relative z-10">
          ...
          <div className="grid md:grid-cols-2 gap-6 max-w-3xl mx-auto mb-12">
            <div className="bg-slate-900/50 p-6 rounded-xl border border-slate-800 text-left">
              <h4 className="font-bold text-lg mb-2 text-white">Custom Agent Personas</h4>
              <p className="text-slate-400 text-sm">Design agents that mimic your best analysts' behavior and decision-making logic.</p>
            </div>
            <div className="bg-slate-900/50 p-6 rounded-xl border border-slate-800 text-left">
              <h4 className="font-bold text-lg mb-2 text-white">Workflow Automation</h4>
              <p className="text-slate-400 text-sm">Automate specific SOPs and repetitive investigations unique to your organization.</p>
            </div>
          </div>
...
export default Strike48Presentation;